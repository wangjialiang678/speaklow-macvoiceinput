...
Swift app 通过 HTTP 调用本地 asr-bridge（localhost:18089），asr-bridge 通过 WebSocket 连接阿里云 DashScope FunASR 进行语音识别。识别完成后可选经 LLM（qwen-turbo-latest）优化文字。
...
asr-bridge/            ← Go ASR 桥接服务
  ...
  transcribe.go        ← 批量转录（DashScope WebSocket）
  stream.go            ← 流式转录（实时 WebSocket）
