...
- **插入失败浮窗**：当自动插入失败时，弹出浮窗显示可选中的识别文字，按 ⌘V 粘贴后自动关闭
- **双阶段 ASR**：流式实时预览（paraformer-realtime-v2）+ 松手后二次精准识别（qwen3-asr-flash），自动超时降级
- **AI 文字优化**：识别后由大模型（qwen-turbo-latest）自动纠错、润色，支持按句流式润色减少等待（可关闭）
- **录音波形**：屏幕顶部 notch 区域显示实时波形动画
- **热词支持**：自定义热词表提升专业术语（AI 模型名、框架名等）识别率，含中英混排 target_lang 标注
...
Swift app 通过 HTTP 调用本地 asr-bridge（localhost:18089），asr-bridge 通过 WebSocket 连接阿里云 DashScope FunASR 进行流式识别，松手后通过同步 REST API 调用 qwen3-asr-flash 做二次精准识别。asr-bridge 基于 [audio-asr-suite](../../../组件模块/audio-asr-suite) 的 `pkg/realtime` 和 `pkg/hotword` 模块，支持热词表提升专业术语识别率。识别完成后可选经 LLM（qwen-turbo-latest）优化文字。
...
asr-bridge/            ← Go ASR 桥接服务（基于 audio-asr-suite）
  main.go              ← HTTP 路由（/health, /v1/transcribe, /v1/transcribe-sync, /v1/stream, /v1/refine）
  ...
  hotword.go           ← 热词表初始化与管理（含 vocabularyID 持久化 + SHA256 变更检测）
...
- vocabularyID 持久化：缓存至 `~/.config/speaklow/vocabulary_id`，hotwords.txt 未变时直接复用（SHA256 变更检测）
...
| `ASR_MODEL` | `paraformer-realtime-v2` | 流式 ASR 模型 |
| `ASR_SYNC_MODEL` | `qwen3-asr-flash` | 同步二次识别 ASR 模型 |
