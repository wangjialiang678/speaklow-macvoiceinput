...
- **Always use `make install` to deploy to /Applications/** — `make install` does clean build + kill old process + rm old .app + copy + verify binary + launch + confirm PID. Manual `cp -R` + `open` is unreliable: macOS may keep the old process running, or cache the old binary. `make run` is for dev builds from `build/` only.
...
- **Bluetooth mic silence**: Bluetooth microphones (e.g., HUAWEI FreeBuds) can stop producing audio on macOS due to HFP/SCO profile issues. The app detects this via silence timeout (2s no audio) and shows a device-specific suggestion to switch to built-in mic. If silence timeout fires, all streaming state (WS connection, isStreaming flag) is cleaned up to prevent leaks.
...
- **Microphone permission stale after rebuild**: After `make all` + reinstall, macOS code signature changes and TCC may revoke mic permission silently. `AVAudioEngine.start()` fails with error `2003329396` (`kAUStartIO`). This is NOT a device error — retrying or switching devices won't help. The app detects this error code and shows "麦克风权限需要重新授权" with instructions to toggle the mic permission in System Settings.
- **Bridge port conflict → infinite restart**: If an old bridge still holds port 18089, the new bridge crashes on `bind: address already in use`. The health check hits the OLD bridge → resets restart counter → infinite loop. Fix: `start()` checks port occupancy first, adopts healthy old bridge or kills unhealthy one. Health check only resets counter when OUR process (by PID) is confirmed alive. Additional safeguard: 5-minute crash window — if bridge crashes >10 times within 5 minutes, auto-restart stops entirely. Health monitor no longer resets `consecutiveRestarts`.
- **Duplicate app instances**: Installing to a different path (e.g., `/Applications/` vs `~/Applications/`) and opening creates a second instance. Both register hotkey monitors and bridge managers, causing double events and port conflicts. Fix: single-instance guard in `applicationDidFinishLaunching` uses `exit(0)` (not `NSApp.terminate` which is async and allows remaining init code to run). Always `pkill -f SpeakLow` before launching a new build.
...
- **WebSocket leak on mic failure**: In streaming mode, `_beginRecordingAfterHealthCheck` creates and starts `StreamingTranscriptionService` before starting the mic. If `audioRecorder.startRecording()` fails (e.g., error 2003329396), or silence timeout fires, the WS connection was left orphaned — `showMicError()` only set `isRecording=false` without cleaning up `streamingService`/`isStreaming`. Each retry leaked another WS. Fix: `showMicError()` and `onSilenceTimeout` now disconnect WS, nil service, reset `isStreaming`. `streamingDidFail` guards on `isStreaming` to ignore stale callbacks.
- **Double LLM refine calls**: When `sentence_end` and `finished` arrive near-simultaneously, pre-refine cache isn't populated yet, causing `applyFinalTranscript` to fire a duplicate refine API call. Fix: when `pendingRefineCount > 0`, wait up to 3s for pre-refine to complete before using cache, instead of falling through to a fresh refine.
